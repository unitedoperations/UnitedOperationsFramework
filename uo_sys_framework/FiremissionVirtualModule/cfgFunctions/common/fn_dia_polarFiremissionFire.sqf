	...
	if (_name == lbText [POFM_DIA_IDC_GUNSELECT, lbCurSel POFM_DIA_IDC_GUNSELECT]) then {
		if (_ready) then {
			...
			if(_inputIsCorrect) then {
				...
				[QGVAR(Event_PolarFiremission), [player,_name,_newAmmoClass,_selectedGrid,_selectedMils,_selectedDist,_selectedRnds,_selectedGuns]] call CBA_fnc_serverEvent;
