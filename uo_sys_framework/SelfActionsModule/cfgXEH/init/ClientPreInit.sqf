...
["UO_FW_SettingsLoaded", {
    ...
    if ({_x} count [UO_FW_SelfActions_Enable,UO_FW_SelfActions_CheckMap_Enabled,UO_FW_SelfActions_CutGrass_Enabled] isEqualto 0) exitWith {};
    ["UO_FW_RegisterModuleEvent", ["Self Actions", "Allows players to check their own team color, view other's maps and cut grass.", "TinfoilHate and PiZZADOX"]] call CBA_fnc_globalEvent;
    [{!isNull player}, {
        if (!UO_FW_SelfActions_Enable) exitwith {};
        player setVariable ["UO_FW_SelfActions_originalSide",side player,true];
        if (UO_FW_SelfActions_CheckColour_Enabled) then {
            ...
            [player, 1, ["ACE_SelfActions"], _teamColorAction] call ace_interact_menu_fnc_addActionToObject;
        };
        ...
        if (UO_FW_SelfActions_CutGrass_Enabled) then {
            ...
            [player, 1, ["ACE_SelfActions"], _macheteAction] call ace_interact_menu_fnc_addActionToObject;
        };
