...
_handle = _this spawn {
        ...
        for "_i" from 0 to _salvoCount do {
                _row = 0;
                {
                        [_unit,(_startingSpots select _row) vectorAdd (_dir vectorMultiply _i),0,_salvoSize,_roundClassName] call EFUNC(Firemission,InternalFiremission);
                    ...
                    sleep((((_fireRate select 0) * ((_unit select 0) getVariable [QEGVAR(Firemission,ArtFireRate),MEANFIRERATE])) * _salvoSize) max _salvoWait);
        };
        {
