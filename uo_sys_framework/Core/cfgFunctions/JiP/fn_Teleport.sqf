...
if (player isEqualto _target || !(_target call EFUNC(Core,Alive))) then {
    private _rank = -1;
    {
        if (rankId _x > _rank && {(_target call EFUNC(Core,Alive))}) then {
            _rank = rankId _x;
            _target = _x;
        };
    } forEach ((units (group player)) - [player]);
};
if (!((_target call EFUNC(Core,Alive)) && {((_target isEqualto vehicle _target) || ((vehicle _target) call EFUNC(Core,HasEmptyPositions)))})) then {
    private _rank = -1;
    private _count = 0;
    {
        if (_x call EFUNC(Core,Alive)) then {
            _count = _count + 1;
            if ((rankId _x > _rank) && {(!(_x isEqualto vehicle _x) || ((vehicle _x) call EFUNC(Core,HasEmptyPositions)))}) then {
                _rank = rankId _x;
                _target = _x;
            };
        };
    ...
    if (_rank isEqualto -1) then {
        ...
        if (_count isEqualto 0) then {
            ...
            ["No one left in the squad, cannot teleport!"] call EFUNC(Core,parsedTextDisplay);
        } else {
            ["Not possible to JIP teleport to anyone, please try again later"] call EFUNC(Core,parsedTextDisplay);
        };
    };
};
    ...
    if !(_target isEqualto vehicle _target) then {
        ...
        if (_pos distance _target <= 15) then {
            ...
            if !(_enemy) then {
                ...
                ["Enemies are too close to group leader, please try again later"] call EFUNC(Core,parsedTextDisplay);
            };
        } else {
             LOG_1("Jip Teleport failed due due space %1",_target);
            ["Not possible to JIP teleport to group leader, please try again later"] call EFUNC(Core,parsedTextDisplay);
        };
    };
} else {
    LOG_1("Jip Teleport failed to %1",_target);
    ["Something went wrong, target doesn't exist."] call EFUNC(Core,parsedTextDisplay);
};
