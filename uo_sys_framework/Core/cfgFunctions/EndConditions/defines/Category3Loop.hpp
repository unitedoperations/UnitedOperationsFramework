if (GETMVAR(Enabled_3,false)) then {
    if ((isNil "GVAR(Message_3)") || {!(GVAR(Message_3) isEqualType "STRING")}) then {
        ...
        if (GETMVAR(CasualtyCount_CIV_3,false)) then {_conditionsCountCategory3 = _conditionsCountCategory3 + 1;};
        ...
        if ((_conditionsCountCategory3) > 0) then {
            ...
            private _endConditionsCategory3PFHhandle = [{
                ...
                if (GETMVAR(CasualtyCount_BLUFOR_3,false)) then {
                    ...
                    if (_westCasualty >= GETMVAR(CasualtyCount_BLUFOR_Percentage_3,75)) then {_BluforCasConditionCheck = true;} else {_BluforCasConditionCheck = false;};
                ...
                if (GETMVAR(CasualtyCount_OPFOR_3,false)) then {
                    ...
                    if (_eastCasualty >= GETMVAR(CasualtyCount_OPFOR_Percentage_3,75)) then {_OpforCasConditionCheck = true;} else {_OpforCasConditionCheck = false;};
                ...
                if (GETMVAR(CasualtyCount_INDFOR_3,false)) then {
                    ...
                    if (_resCasualty >= GETMVAR(CasualtyCount_INDFOR_Percentage_3,75)) then {_IndforCasConditionCheck = true;} else {_IndforCasConditionCheck = false;};
                    _ConditionCheckList pushback ["INDFOR Cas Check",_IndforCasConditionCheck];
                };

                private _CIVCasConditionCheck = false;

                if (GETMVAR(CasualtyCount_CIV_3,false)) then {
                    private _civCasualty = EGVAR(Core,TeamName_Civilian) call EFUNC(Core,CasualtyPercentage);
                    if (_civCasualty >= GETMVAR(CasualtyCount_CIV_Percentage_3,75)) then {_CIVCasConditionCheck = true;} else {_CIVCasConditionCheck = false;};
                ...
                if (GETMVAR(ExtractionEnabled_3,false)) then {
                    private _team = ([EGVAR(Core,TeamName_Blufor),EGVAR(Core,TeamName_Opfor),EGVAR(Core,TeamName_Indfor),EGVAR(Core,TeamName_Civilian)] select GVAR(ExtractionTeam_3));
                    if (GVAR(ExtractionMarker_3) isEqualto "") exitwith {["","No marker entered for extract zone for Category 3!"] call EFUNC(Core,DebugMessageDetailed);};
                    if ((getMarkerColor GVAR(ExtractionMarker_3)) isEqualto "") exitwith {["","Invalid extract marker for Category 3!"] call EFUNC(Core,DebugMessageDetailed);};
