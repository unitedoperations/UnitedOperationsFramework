if (GETMVAR(Enabled_5,false)) then {
    if ((isNil "GVAR(Message_5)") || {!(GVAR(Message_5) isEqualType "STRING")}) then {
        ...
        if (GETMVAR(CasualtyCount_CIV_5,false)) then {_conditionsCountCategory5 = _conditionsCountCategory5 + 1;};
        ...
        if ((_conditionsCountCategory5) > 0) then {
            ...
            private _endConditionsCategory5PFHhandle = [{
                ...
                if (GETMVAR(CasualtyCount_BLUFOR_5,false)) then {
                    ...
                    if (_westCasualty >= GETMVAR(CasualtyCount_BLUFOR_Percentage_5,75)) then {_BluforCasConditionCheck = true;} else {_BluforCasConditionCheck = false;};
                ...
                if (GETMVAR(CasualtyCount_OPFOR_5,false)) then {
                    ...
                    if (_eastCasualty >= GETMVAR(CasualtyCount_OPFOR_Percentage_5,75)) then {_OpforCasConditionCheck = true;} else {_OpforCasConditionCheck = false;};
                ...
                if (GETMVAR(CasualtyCount_INDFOR_5,false)) then {
                    ...
                    if (_resCasualty >= GETMVAR(CasualtyCount_INDFOR_Percentage_5,75)) then {_IndforCasConditionCheck = true;} else {_IndforCasConditionCheck = false;};
                    _ConditionCheckList pushback ["INDFOR Cas Check",_IndforCasConditionCheck];
                };

                private _CIVCasConditionCheck = false;

                if (GETMVAR(CasualtyCount_CIV_5,false)) then {
                    private _civCasualty = EGVAR(Core,TeamName_Civilian) call EFUNC(Core,CasualtyPercentage);
                    if (_civCasualty >= GETMVAR(CasualtyCount_CIV_Percentage_5,75)) then {_CIVCasConditionCheck = true;} else {_CIVCasConditionCheck = false;};
