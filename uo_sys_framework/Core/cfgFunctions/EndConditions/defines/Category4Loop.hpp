if (missionNamespace getVariable ["UO_FW_EndCondition_Enabled_4",false]) then {
    if ((isNil "UO_FW_EndCondition_Message_4") || {!(UO_FW_EndCondition_Message_4 isEqualType "STRING")}) then {
        ...
        if (UO_FW_EndCondition_CasualtyCount_INDFOR_Enabled_4) then {_conditionsCountCategory4 = _conditionsCountCategory4 + 1;};
        if (UO_FW_EndCondition_CasualtyCount_CIV_Enabled_4) then {_conditionsCountCategory4 = _conditionsCountCategory4 + 1;};
        ...
        if ((_conditionsCountCategory4) > 0) then {
            ...
            private _endConditionsCategory4PFHhandle = [{
                ...
                if (UO_FW_EndCondition_CasualtyCount_INDFOR_Enabled_4) then {
                    private _resCasualty = UO_FW_TeamSetting_TeamName_Indfor call UO_FW_fnc_CasualtyPercentage;
                    if (_resCasualty >= UO_FW_EndCondition_CasualtyCount_INDFOR_Percentage_4) then {_IndforCasConditionCheck = true;} else {_IndforCasConditionCheck = false;};
                    _ConditionCheckList pushback ["INDFOR Cas Check",_IndforCasConditionCheck];
                };

                private _CIVCasConditionCheck = false;

                if (UO_FW_EndCondition_CasualtyCount_CIV_Enabled_4) then {
                    private _civCasualty = UO_FW_TeamSetting_TeamName_Civ call UO_FW_fnc_CasualtyPercentage;
                    if (_civCasualty >= UO_FW_EndCondition_CasualtyCount_CIV_Percentage_4) then {_CIVCasConditionCheck = true;} else {_CIVCasConditionCheck = false;};
                    _ConditionCheckList pushback ["CIVILIAN Cas Check",_CIVCasConditionCheck];
                };

                //alive entity block
                private _aliveUnitArray = missionNamespace getVariable ["UO_FW_EndCondition_EntitiesAlive_Array_4",[]];
                ...
                //dead entity block
                private _deadUnitArray = missionNamespace getVariable ["UO_FW_EndCondition_EntitiesDead_Array_4",[]];
                ...
                private _damagedUnitArray = missionNamespace getVariable ["UO_FW_EndCondition_EntitiesDamaged_Array_4",[]];
                ...
                private _hostageRescuedArray = missionNamespace getVariable ["UO_FW_EndCondition_HostageRescued_Array_4",[]];
                ...
                private _customVariablesArray = missionNamespace getVariable ["UO_FW_EndCondition_CustomVariables_Array_4",[]];
                if (!(_customVariablesArray isEqualto [])) then {
                    LOG_1("Var Array 1:%1",UO_FW_EndCondition_CustomVariables_Array_4);
                ...
                private _captureZones_Array_4 = missionNamespace getVariable ["UO_FW_EndCondition_CaptureZoneCaptured_Array_4",[]];
                if (!(_captureZones_Array_4 isEqualto [])) then {
                    private _captureZones_TeamSetting_4 = missionNamespace getVariable ["UO_FW_EndCondition_CaptureZoneCaptured_Team_4",1];
                    {
                ...
                if (UO_FW_EndCondition_ExtractionEnabled_4) then {
                    private _team = ([UO_FW_TeamSetting_TeamName_Blufor,UO_FW_TeamSetting_TeamName_Opfor,UO_FW_TeamSetting_TeamName_Indfor,UO_FW_TeamSetting_TeamName_Civ] select UO_FW_EndCondition_ExtractionTeam_4);
