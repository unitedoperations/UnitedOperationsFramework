if (GETMVAR(Enabled_2,false)) then {
    if ((isNil "GVAR(Message_2)") || {!(GVAR(Message_2) isEqualType "STRING")}) then {
        ...
        if (GETMVAR(CasualtyCount_Civilian_2,false)) then {_conditionsCountCategory2 = _conditionsCountCategory2 + 1;};
        ...
        if ((_conditionsCountCategory2) > 0) then {
            ...
            private _endConditionsCategory2PFHhandle = [{
                ...
                if (GETMVAR(CasualtyCount_Civilian_2,false)) then {
                    private _civCasualty = EGVAR(Core,TeamName_Civilian) call EFUNC(Core,CasualtyPercentage);
                    if (_civCasualty >= GETMVAR(CasualtyCount_Percentage_Civilian_2,75)) then {_CasConditionCheck_Civilian = true;} else {_CasConditionCheck_Civilian = false;};
