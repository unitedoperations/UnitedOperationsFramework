if (GETMVAR(Enabled_2,false)) then {
    if ((isNil "GVAR(Message_2)") || {!(GVAR(Message_2) isEqualType "STRING")}) then {
        ...
        if (GETMVAR(CasualtyCount_BLUFOR_2,false)) then {_conditionsCountCategory2 = _conditionsCountCategory2 + 1;};
        if (GETMVAR(CasualtyCount_OPFOR_2,false)) then {_conditionsCountCategory2 = _conditionsCountCategory2 + 1;};
        if (GETMVAR(CasualtyCount_INDFOR_2,false)) then {_conditionsCountCategory2 = _conditionsCountCategory2 + 1;};
        if (GETMVAR(CasualtyCount_CIV_2,false)) then {_conditionsCountCategory2 = _conditionsCountCategory2 + 1;};
        ...
        if ((_conditionsCountCategory2) > 0) then {
            ...
            private _endConditionsCategory2PFHhandle = [{
                ...
                if (GETMVAR(CasualtyCount_BLUFOR_2,false)) then {
                    private _westCasualty = EGVAR(Core,TeamName_Blufor) call EFUNC(Core,CasualtyPercentage);
                    if (_westCasualty >= GETMVAR(CasualtyCount_BLUFOR_Percentage_2,75)) then {_BluforCasConditionCheck = true;} else {_BluforCasConditionCheck = false;};
                    _ConditionCheckList pushback ["BLUFOR Cas Check",_BluforCasConditionCheck];
                };

                private _OpforCasConditionCheck = false;

                if (GETMVAR(CasualtyCount_OPFOR_2,false)) then {
                    private _eastCasualty = EGVAR(Core,TeamName_Opfor) call EFUNC(Core,CasualtyPercentage);
                    if (_eastCasualty >= GETMVAR(CasualtyCount_OPFOR_Percentage_2,75)) then {_OpforCasConditionCheck = true;} else {_OpforCasConditionCheck = false;};
                    _ConditionCheckList pushback ["OPFOR Cas Check",_OpforCasConditionCheck];
                };

                private _IndforCasConditionCheck = false;

                if (GETMVAR(CasualtyCount_INDFOR_2,false)) then {
                    private _resCasualty = EGVAR(Core,TeamName_Indfor) call EFUNC(Core,CasualtyPercentage);
                    if (_resCasualty >= GETMVAR(CasualtyCount_INDFOR_Percentage_2,75)) then {_IndforCasConditionCheck = true;} else {_IndforCasConditionCheck = false;};
                    _ConditionCheckList pushback ["INDFOR Cas Check",_IndforCasConditionCheck];
                };

                private _CIVCasConditionCheck = false;

                if (GETMVAR(CasualtyCount_CIV_2,false)) then {
                    private _civCasualty = EGVAR(Core,TeamName_Civilian) call EFUNC(Core,CasualtyPercentage);
                    if (_civCasualty >= GETMVAR(CasualtyCount_CIV_Percentage_2,75)) then {_CIVCasConditionCheck = true;} else {_CIVCasConditionCheck = false;};
                ...
                if (!(_customVariablesArray isEqualto [])) then {
                    LOG_1("Var Array 1:%1",GETMVAR(CustomVariables_2,[]));
