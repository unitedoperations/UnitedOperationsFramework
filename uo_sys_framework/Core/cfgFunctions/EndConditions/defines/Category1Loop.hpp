if (GETMVAR(Enabled_1,false)) then {
    if ((isNil QGVAR(Message_1)) || {!(GETMVAR(Message_1,"") isEqualType "STRING")}) then {
        ...
        if (GETMVAR(CasualtyCount_BLUFOR_Enabled_1,false)) then {_conditionsCountCategory1 = _conditionsCountCategory1 + 1;};
        if (GETMVAR(CasualtyCount_OPFOR_Enabled_1,false)) then {_conditionsCountCategory1 = _conditionsCountCategory1 + 1;};
        if (GETMVAR(CasualtyCount_INDFOR_Enabled_1,false)) then {_conditionsCountCategory1 = _conditionsCountCategory1 + 1;};
        if (GETMVAR(CasualtyCount_CIV_Enabled_1,false)) then {_conditionsCountCategory1 = _conditionsCountCategory1 + 1;};
        ...
        if ((_conditionsCountCategory1) > 0) then {
            ...
            private _endConditionsCategory1PFHhandle = [{
                ...
                if (GETMVAR(CasualtyCount_BLUFOR_Enabled_1,false)) then {
                    private _westCasualty = EGVAR(Core,TeamName_Blufor) call EFUNC(Core,CasualtyPercentage);
                    if (_westCasualty >= GETMVAR(CasualtyCount_BLUFOR_Percentage_Enabled_1,75)) then {_BluforCasConditionCheck = true;} else {_BluforCasConditionCheck = false;};
                    _ConditionCheckList pushback ["BLUFOR Cas Check",_BluforCasConditionCheck];
                };

                private _OpforCasConditionCheck = false;

                if (GETMVAR(CasualtyCount_OPFOR_Enabled_1,false)) then {
                    private _eastCasualty = EGVAR(Core,TeamName_Opfor) call EFUNC(Core,CasualtyPercentage);
                    if (_eastCasualty >= GETMVAR(CasualtyCount_OPFOR_Percentage_Enabled_1,75)) then {_OpforCasConditionCheck = true;} else {_OpforCasConditionCheck = false;};
                    _ConditionCheckList pushback ["OPFOR Cas Check",_OpforCasConditionCheck];
                };

                private _IndforCasConditionCheck = false;

                if (GETMVAR(CasualtyCount_INDFOR_Enabled_1,false)) then {
                    private _resCasualty = EGVAR(Core,TeamName_Indfor) call EFUNC(Core,CasualtyPercentage);
                    if (_resCasualty >= GETMVAR(CasualtyCount_INDFOR_Percentage_Enabled_1,75)) then {_IndforCasConditionCheck = true;} else {_IndforCasConditionCheck = false;};
                    _ConditionCheckList pushback ["INDFOR Cas Check",_IndforCasConditionCheck];
                };

                private _CIVCasConditionCheck = false;

                if (GETMVAR(CasualtyCount_CIV_Enabled_1,false)) then {
                    private _civCasualty = EGVAR(Core,TeamName_Civilian) call EFUNC(Core,CasualtyPercentage);
                    if (_civCasualty >= GETMVAR(CasualtyCount_CIV_Percentage_Enabled_1,75)) then {_CIVCasConditionCheck = true;} else {_CIVCasConditionCheck = false;};
                ...
                if (!(_customVariablesArray isEqualto [])) then {
                    LOG_1("Var Array 1:%1",GETMVAR(CustomVariables_1,[]));
                ...
                if (GETMVAR(ExtractionEnabled_1,false)) then {
                    private _team = ([EGVAR(Core,TeamName_Blufor),EGVAR(Core,TeamName_Opfor),EGVAR(Core,TeamName_Indfor),EGVAR(Core,TeamName_Civilian)] select GVAR(ExtractionTeam_1));
                    if (GVAR(ExtractionMarker_1) isEqualto "") exitwith {["","No marker entered for extract zone for Category 1!"] call EFUNC(Core,DebugMessageDetailed);};
                    if ((getMarkerColor GVAR(ExtractionMarker_1)) isEqualto "") exitwith {["","Invalid extract marker for Category 1!"] call EFUNC(Core,DebugMessageDetailed);};
