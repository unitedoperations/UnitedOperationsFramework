            ...
            };
            {
                if (_thisGroup knowsAbout _x > _thatGroup knowsAbout _x && {_thisGroup knowsAbout _x > 1} && {_thatGroup knowsAbout _x < 1.5} && {(leader _thisGroup distance2D _x) <= _range}) then {
                    ...
                    [{
                        ...
                        if (GVAR(debug)) then {
                            LOG_4("Update || This: %1 | That: %2 | Targ: %3 | Knows:%4",_thisGroup,_thatGroup,_targUnit,_revAmt);
                        };
                    }, [_thatGroup,_thisGroup,_x],_waitTime] call CBA_fnc_waitAndExecute;
                    if (GVAR(debug)) then {
                        LOG_3("Pre-update || This: %1 | That: %2 | Targ: %3",_thisGroup,_thatGroup,_x);
                    };
                };
                if (GVAR(debug)) then {
                    LOG_3("Check || This: %1 | That: %2 | Targ: %3",_thisGroup knowsAbout _x,_thatGroup knowsAbout _x,_x);
                };
            } forEach ([allUnits,playableUnits] select isMultiplayer);

            if (GVAR(debug)) then {
                diag_log format["Rem Groups: %1 | This: %2 | That:%3 | ThisLR:%4 | ThatLR:%5 | Range:%6",_groups,_thisGroup,_thatGroup,_thisLR,_thatLR,_range];
            };
        };
    } forEach _groups;
