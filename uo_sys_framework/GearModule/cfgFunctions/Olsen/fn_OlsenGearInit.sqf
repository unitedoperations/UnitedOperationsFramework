...
[] spawn {
	...
	if (!isDedicated && hasinterface) then {
		...
		if !(_type isEqualto "") then {
			...
			private _msg = format ["Executing gear of file: %1 for unit %2",(player getVariable ["UO_FW_Gear_Olsen_UnitGroupname",""]),player];
			UO_FW_DEBUG("",_msg);
	...
	if(isServer) then {
		{
			if(!(isPlayer _x)) then {
				if !((_x getVariable ["UO_FW_Gear_Olsen_UnitGroupname",""]) isEqualto "" ) then {
					(group _x) setGroupId [(_x getVariable ["UO_FW_Gear_Olsen_UnitGroupname",""])];
				};
