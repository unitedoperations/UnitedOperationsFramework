...
diag_log format["RETURN group alive | %1",count ((units _grp) select {alive _x}) > 0];
	if( count ((units _grp) select {alive _x}) > 0) then {
		sleep (_grp getVariable ['UO_FW_waypointWait',10]);
	...
	diag_log format["RETURN _playerDistances | %1",_playerDistances];
		if(count _playerDistances >= 0) then {
			...
			_grp setVariable["UO_FW_SavedWaypoints",([waypoints _leader] call UO_FW_AI_fnc_getWaypointDetails)];
	diag_log format["RETURN distance target | %1",_target distanceSqr _leader];
		...
		[_grp] call CBA_fnc_clearWaypoints;
		/*if(count (_grp getVariable ['UO_FW_SavedWaypoints',[]]) > 2) then {
			[_grp,(_grp getVariable ['UO_FW_SavedWaypoints',[]])] call UO_FW_AI_fnc_createWaypoints;
		} else {
			if(count (_grp getVariable ['UO_FW_tasks',[]]) > 0 && (_grp getVariable ['UO_FW_TaskTimer',0]) isEqualTo 0) then {
				[_grp,(_grp getVariable ['UO_FW_tasks',[]])] call UO_FW_AI_fnc_taskInit;
			} else {
				_grp setVariable["UO_FW_CurrentTaskEndTime",(time + (_grp getVariable ['UO_FW_TaskTimer',0]))];
				[(_grp getVariable ['UO_FW_task',0]),_grp,(_grp getVariable ['UO_FW_GroupPos',getPos _leader]),(_grp getVariable ['UO_FW_patrolRadius',100]),(_grp getVariable ['UO_FW_waypointWait',3]),(_grp getVariable ['UO_FW_behaviour',"AWARE"]),(_grp getVariable ['UO_FW_combatMode',"RED"]),(_grp getVariable ['UO_FW_speed',"FULL"]),(_grp getVariable ['UO_FW_formation',"WEDGE"]),(_grp getVariable ['UO_FW_occupyOption',0])] call UO_FW_AI_fnc_taskAssign;
			};
		};*/
