			...
			if !is3DEN then {
				...
				private _isRectangle = if((typeof _logic) isEqualTo "UO_FW_AI_ZoneModule_R") then {true} else {false};
					...
					UO_FW_zones pushBack [
						...
						([_logic getVariable ["aeZoneSide",4]] call UO_FW_AI_fnc_getSide),
					...
					if((count UO_FW_zoneInit) >= (count (entities [["UO_FW_AI_ZoneModule_R","UO_FW_AI_ZoneModule"],[]])) && !UO_FW_templateCleanup) then {
						UO_FW_templateSyncedObjects = UO_FW_templateSyncedObjects arrayIntersect UO_FW_templateSyncedObjects;
						[UO_FW_templateSyncedObjects] call UO_FW_AI_fnc_deleteVehicles;
				...
				if (_initial) then {
					[_logic,0,_code,{},_initial] spawn UO_FW_AI_fnc_setup;
