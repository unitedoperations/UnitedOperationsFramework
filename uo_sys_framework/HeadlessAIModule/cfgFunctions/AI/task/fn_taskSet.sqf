...
params ["_grp",["_newTask",objNull,[objNull]],["_setTime",0,[0]]];
	if(!isNull _newTask) then {
		(_newTask call UO_FW_AI_fnc_getTaskParams) params ["_newTask","_cond","_prior","_time","_onComp","_aeTaskId","_radius","_task","_occupy","_wait","_behave","_combat","_speed","_form","_stance"];
		...
		if(!isNull (_grp getVariable["aeCurrentTask",objNull])) then {
			[_grp,(_grp getVariable["aeCurrentTask",objNull])] call UO_FW_AI_fnc_taskComplete;
		};
		...
		[_task,_grp,(getPosATL _newTask),_radius,_wait,_newBehave,_newCombat,_newSpeed,_newForm,_occupy] call UO_FW_AI_fnc_taskAssign;
	};
	true
