...
params ["_unit",["_bld",objNull,[objNull]],["_bldPos",[],[[]]],["_wpWait",5,[0]],["_uSet",[],[[]]],["_sec",[],[[]]],["_error",false,[false]],["_m",0,[0]],"_i"];
	...
	while {alive _unit && !_error && (_unit getvariable["aeOccupy",true]) && (_unit getvariable["aeOccupiedBuilding",objNull]) isEqualTo _bld} do {
		private _pos = _bldPos select (floor (random (count _bldPos)));
		private _timer = 60;
		_unit doMove _pos;
		sleep 5;
		while {alive _unit && _timer > 0 && ((getPosATL _unit) distance _pos) > 2 && (_unit getvariable["aeOccupy",true]) && (_unit getvariable["aeOccupiedBuilding",objNull]) isEqualTo _bld} do {
			sleep 1;
			_timer = _timer - 1;
		};
		if(_unit getvariable["aeOccupy",true] && (_unit getvariable["aeOccupiedBuilding",objNull]) isEqualTo _bld) then {
			doStop _unit;
