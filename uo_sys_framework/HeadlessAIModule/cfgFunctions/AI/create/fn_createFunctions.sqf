...
sleep _delay;
{
	...
	if( count _posModules > 0) then {
		...
		if( ((_posModules select 0) getVariable ['aePositionRadius',0]) > 0) then {
			_spos = [_pos,0,((_posModules select 0) getVariable ['aePositionRadius',0]),1] call UO_FW_AI_fnc_getRandomPositionCircle;
		} else {
			_spos = _pos;
		};
	};
	switch (typeof _logic) do {
		...
		case "UO_FW_AI_AirDropModule": {
			[	(getPosATL _logic),
				...
				_logic getVariable ['aeADOrdnanceType',"Box_NATO_Ammo_F"],
