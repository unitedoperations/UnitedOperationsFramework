...
if (count _blds < 1) then {
    _blds = [_pos,_radius,(count (units _grp))] call UO_FW_AI_fnc_getNearestBuildings;
    if (count _blds < 1) then {
        _bpos = _pos;
        _patrol = true;
    } else {
...
if (UO_FW_AI_DEBUG) then {
    if (_patrol) then {
        ...
        {
            [_grp,(getPosATL _x),format["gBldD%1",_forEachIndex]] call UO_FW_AI_fnc_debugCreateMarker;
        } forEach _blds;
    };
};
