...
params ["_grp",["_newTask",objNull,[objNull]],["_setTime",0,[0]]];
	if(!isNull _newTask) then {
		...
		_grp setVariable["UO_FW_CurrentTaskEndTime",(time + _setTime)];
		if(!isNull (_grp getVariable["UO_FW_CurrentTask",objNull])) then {
			[_grp,(_grp getVariable["UO_FW_CurrentTask",objNull])] call UO_FW_AI_fnc_taskComplete;
		};
		_grp setVariable["UO_FW_CurrentTask",_newTask];
