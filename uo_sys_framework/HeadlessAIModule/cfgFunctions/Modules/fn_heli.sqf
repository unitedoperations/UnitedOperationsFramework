		...
		case "init": {
			if !is3DEN then {
				...
				for [{_t=0}, {(_t < count _synced)}, {_t = _t + 1}] do {
					...
					private _gx = (_logic getVariable ['UO_FW_hiMultiplier',1]);
					private _radius = (_logic getVariable ['UO_FW_hiRadius',0]);
					if (_obj isKindOf "Logic") then {
						...
						for [{_i = (count _posModules) -1}, {_i >= 0}, {_i = _i - 1}] do {
							...
							_msg = format["RETURN UO_FW_positionType | %1 : %2",_module getVariable ['UO_FW_positionType',0],_module];
							UO_FW_DEBUG("",_msg)
							if ((_module getVariable ['UO_FW_positionType',0]) isEqualTo 1) then {
						...
						if (_obj isKindOf "Man") then {
							...
							if (_grpldr == _obj) then {
								...
								private _gx = _logic getVariable ['UO_FW_multiplier',1];
								...
								for [{_j=0}, {(_j < (count _units))}, {_j = _j + 1}] do {
									...
									if (!isNull _veh) then {
										...
										if(!(_veh in _vehLog)) then {
											private _unitInit = _veh getVariable ["UO_FW_unitInit",""];
											...
											(_group select 2) pushBack [
												...
												_veh getVariable ["UO_FW_unitName",""],
												_veh getVariable ['UO_FW_unitPersistent',false],
												_unitInit
											];
									...
									if(_grpldr == _u) then {
										_grpInit = _grp getVariable ["UO_FW_groupInit","true"];
										_occupy = _grp getVariable ['UO_FW_occupyOption',0];
