...
params ["_grpid","_grpSet","_grpMem",["_blds",[],[[]]],["_bldPos",[],[[]]],["_uBld",objNull,[objNull]]];
	...
	private _spos = _gpos;
	_blds = [_spos,_taskRadius,_occupyOption,(count _grpMem)] call UO_AI_fnc_getBuildingList;
	...
	for [{_i=0},{(_i < (count _grpMem))},{_i = _i + 1}] do {
		...
		private _u = [true,_ngrp,_spos,_startBld,_i,(_grpMem select _i),_taskRadius] call UO_AI_fnc_createUnit;
	...
	if(count _tasks > 0 && _taskTimer isEqualTo 0) then {
		...
		if(_task isEqualTo 0 || _task isEqualTo 1 || _task isEqualTo 3) then {		
			{_x setvariable["aeOccupy",true]} forEach (units _ngrp);
			_ngrp setVariable["aeCurrentTaskEndTime",(time + _taskTimer)];
			[_task,_ngrp,_spos,_taskRadius,_wait,_behave,_combat,_speed,_formation] call UO_AI_fnc_taskAssign;
		};
	};
