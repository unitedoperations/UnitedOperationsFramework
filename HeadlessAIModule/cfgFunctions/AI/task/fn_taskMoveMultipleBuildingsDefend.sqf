/*	Description: Task a group to move to multiple buildings and take up positions inside.
 * 	Arguments:
...
params[["_blds",[],[[]]],"_grp","_pos",["_radius",0,[0]],["_wait",3,[0]],["_behave","SAFE",[""]],["_combat","RED",[""]],["_speed","LIMITED",[""]],["_formation","WEDGE",[""]],["_type","MOVE",[""]],["_oncomplete","",[""]],["_compradius",0,[0]],["_bldPos",[],[[]]],["_patrol",false,[false]]];
	...
	if(count _blds < 1) then {
		_blds = [_pos,_radius,(count (units _grp))] call UO_AI_fnc_getNearestBuildings;
		if(count _blds < 1) then {
			_bpos = _pos;
			_patrol = true;
		} else {
	...
	if(_patrol) then {
		...
		while {{alive _x} count (units _grp) <= ((count (units _grp)) * 0.5) || (((getPosATL leader _grp) distance _pos) > 30)} do { sleep 5; };	
		{
			...
			[_x,_uBld,_bldPos,_wait,[_behave,_combat,_speed,_formation]] spawn UO_AI_fnc_taskBuildingDefend;
		} forEach (units _grp);
	};
	if(UO_FW_AI_DEBUG) then {
		if(_patrol) then {
			...
			{
				[_grp,(getPosATL _x),format["gBldD%1",_forEachIndex]] spawn UO_AI_fnc_debugCreateMarker;
			} forEach _blds;
		};
	};
	true