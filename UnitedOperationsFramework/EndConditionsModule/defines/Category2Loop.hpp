...
{
	if ((isNil "UO_FW_EndCondition_Message_2") || !(typename UO_FW_EndCondition_Message_2 isEqualto "STRING")) then 
	{
		["","invalid message for End Condition Category 2"] call UO_FW_fnc_DebugMessageDetailed;
		
	} else 
	{
		["","Valid message for End Condition Category 2, executing"] call UO_FW_fnc_DebugMessageDetailed;
		//_conditionsCategory2 = [[UO_FW_EndCondition_CasualtyCount_BLUFOR_Enabled_2,UO_FW_EndCondition_CasualtyCount_BLUFOR_Percentage_2],[UO_FW_EndCondition_CasualtyCount_OPFOR_Enabled_2,UO_FW_EndCondition_CasualtyCount_OPFOR_Percentage_2],[UO_FW_EndCondition_CasualtyCount_RESISTANCE_Enabled_2,UO_FW_EndCondition_CasualtyCount_RESISTANCE_Percentage_2],[UO_FW_EndCondition_CasualtyCount_CIVILIAN_Enabled_2,UO_FW_EndCondition_CasualtyCount_CIVILIAN_Percentage_2],[UO_FW_EndCondition_EntitiesAlive_Array_2,UO_FW_EndCondition_EntitiesDead_Array_2,UO_FW_EndCondition_EntitiesDamaged_Array_2,UO_FW_EndCondition_EntitiesDeadORDamaged_Array_2],[UO_FW_EndCondition_CustomVariables_Array_2]];
		//casualties index 0 
		...
		if ((UO_FW_EndCondition_CapturezoneEnabled_2)) then {_conditionsCountCategory2 = _conditionsCountCategory2 + 1;};
		...
		if ((_conditionsCountCategory2) > 0) then 
		{
			...
			[] spawn {
				while {!UO_FW_MissionEnded} do 
				{
					...
					if (UO_FW_EndCondition_CasualtyCount_RESISTANCE_Enabled_2) then 
					{
						_resCasualty = UO_FW_TeamSetting_Indfor_TeamName call UO_FW_fnc_CasualtyPercentage;
						if (_resCasualty >= UO_FW_EndCondition_CasualtyCount_RESISTANCE_Percentage_2) then {_IndforCasConditionCheck = true;} else {_IndforCasConditionCheck = false;};
						_ConditionCheckList pushback ["INDFOR Cas Check",_IndforCasConditionCheck];
					};
					
					_CivilianCasConditionCheck = false;
					
					if (UO_FW_EndCondition_CasualtyCount_CIVILIAN_Enabled_2) then 
					{
						_civCasualty = UO_FW_TeamSetting_Civ_TeamName call UO_FW_fnc_CasualtyPercentage;
						if (_civCasualty >= UO_FW_EndCondition_CasualtyCount_CIVILIAN_Percentage_2) then {_CivilianCasConditionCheck = true;} else {_CivilianCasConditionCheck = false;};
						_ConditionCheckList pushback ["CIVILIAN Cas Check",_CivilianCasConditionCheck];
					};
					
					//alive entity block
					_aliveUnitArray = missionNamespace getVariable ["UO_FW_EndCondition_EntitiesAlive_Array_2",[]];
					...
					_damagedUnitArray = missionNamespace getVariable ["UO_FW_EndCondition_EntitiesDamaged_Array_2",[]];
					...
					_hostageRescuedArray = missionNamespace getVariable ["UO_FW_EndCondition_HostageRescued_Array_2",[]];
					...
					if (UO_FW_EndCondition_CapturezoneEnabled_2) then 
					{
						_type = UO_FW_EndCondition_CapturezoneMarker_2 call UO_FW_FNC_IsCapturezoneCaptured;
						
						_CapturezoneConditionCheck = (_type == (["NOONE","WEST","EAST","RESISTANCE","CIVILIAN"] select UO_FW_EndCondition_CapturezoneTeam_2));
						
						_ConditionCheckList pushback ["CapturezoneCheck",_CapturezoneConditionCheck];
					};

					_tempdebugtext2 = format ["Category 2 _ConditionCheckList:%1",_ConditionCheckList];
					...
					{
						_team = [UO_FW_TeamSetting_Blufor_TeamName,UO_FW_TeamSetting_Opfor_TeamName,UO_FW_TeamSetting_Indfor_TeamName,UO_FW_TeamSetting_Civ_TeamName] select UO_FW_EndCondition_ExtractionTeam_2;
					...
					if (_ExtractionCheck) then 
					{
						...
						{
							{
								...
								if (_value) exitwith 
								{
										_tempdebugtext2 = format ["Category 2 Ending due to :%1",_value];
										["",_tempdebugtext2] call UO_FW_fnc_DebugMessageDetailed;
						...
						else 
						{
							...
							{
								_tempdebugtext2 = format ["Category 2 Ending due to :%1",_CapturezoneConditionCheck];
								["",_tempdebugtext2] call UO_FW_fnc_DebugMessageDetailed;
			...
			["","No Conditions for Category 2"] call UO_FW_fnc_DebugMessageDetailed;
		};
	};
};