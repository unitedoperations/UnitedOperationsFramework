if (missionNamespace getVariable ["UO_FW_EndCondition_Enabled_4",false]) then
{
	...
	} else 
	{
		...
		if ((UO_FW_EndCondition_CapturezoneEnabled_4)) then {_conditionsCountCategory4 = _conditionsCountCategory4 + 1;};
			...
			[] spawn {
				while {!UO_FW_MissionEnded} do 
				{
					...
					if (!(_aliveUnitArray isEqualto [])) then 
					{
						...
						["",format ["Alive Array 4:%1",_aliveUnitArray]] call UO_FW_fnc_DebugMessageDetailed;
						_alive = true;
						{
					...
					{
						["",format ["Dead Array 4:%1",_deadUnitArray]] call UO_FW_fnc_DebugMessageDetailed;
						_dead = true;
						{
							_unit = missionNamespace getVariable [_x,objNull];
							if(typename _unit == "OBJECT" && _unit != objNull) then 
					...
					if (!(_damagedUnitArray isEqualto [])) then 
					{
						...
						["",format ["Damaged Array 4:%1",_damagedUnitArray]] call UO_FW_fnc_DebugMessageDetailed;
					...
					if (!(_hostageRescuedArray isEqualto [])) then 
					{
						...
						["",format ["Rescued Array 4:%1",_hostageRescuedArray]] call UO_FW_fnc_DebugMessageDetailed;
					...
					{
						_tempdebugtext3 = format ["Var Array 4:%1",UO_FW_EndCondition_CustomVariables_Array_4];
					...
					if (UO_FW_EndCondition_CapturezoneEnabled_4) then 
					{
						_type = UO_FW_EndCondition_CapturezoneMarker_4 call UO_FW_FNC_IsCapturezoneCaptured;
						
						_CapturezoneConditionCheck = (_type == (["NOONE","WEST","EAST","RESISTANCE","CIVILIAN"] select UO_FW_EndCondition_CapturezoneTeam_4));
					...
					{
						if (UO_FW_EndCondition_Mode_4 == 1) then 
						{
							{
								_x params ["_name","_value"];
								if (_value) exitwith 
								{
										_tempdebugtext2 = format ["Category 4 Ending due to :%1",_value];
										["",_tempdebugtext2] call UO_FW_fnc_DebugMessageDetailed;
						...
						else 
						{
							...
							{
								_tempdebugtext2 = format ["Category 4 Ending due to :%1",str _ConditionCheckList];
								["",_tempdebugtext2] call UO_FW_fnc_DebugMessageDetailed;
