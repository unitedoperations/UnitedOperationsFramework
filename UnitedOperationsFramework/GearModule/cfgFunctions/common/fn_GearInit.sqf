...
"" spawn
{
	...
	if(!isDedicated) then 
	{
		...
		if(_type != "") then 
		{
			...
			if(player getVariable ["UO_FW_Gear_Groupname",""] != "" ) then {	(group player) setGroupId [_groupId];};
	...
	{
		{
			if(!(isPlayer _x)) then 
			{
				...
				{
					_array = [_x,_type];
					if(_x getVariable ["UO_FW_Gear_Groupname",""] != "" ) then {	(group _x) setGroupId [_groupId];};
					_array call UO_FW_FNC_GearScript;
				};
			};
		}forEach allUnits;
