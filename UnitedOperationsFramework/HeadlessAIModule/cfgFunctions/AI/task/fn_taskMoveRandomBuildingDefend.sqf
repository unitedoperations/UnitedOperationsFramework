...
params[["_bld",objNull,[objNull]],"_grp","_pos",["_radius",0,[0]],["_wait",3,[0]],["_behave","SAFE",[""]],["_combat","RED",[""]],["_speed","LIMITED",[""]],["_formation","WEDGE",[""]],["_type","MOVE",[""]],["_oncomplete","",[""]],["_compradius",0,[0]],["_bldPos",[],[[]]],["_patrol",false,[false]]];
	...
	private _bpos = _pos;
	if(isNull _bld) then {
		_bld = [_pos,_radius] call UO_AI_fnc_getRandomBuilding;
		if(isNull _bld) then {
			_bpos = _pos;
			_patrol = true;
		} else {
	...
	if(_patrol) then {
		...
		while {{alive _x} count (units _grp) <= ((count (units _grp)) * 0.5) || (((getPosATL (leader _grp)) distance _pos) > 50)} do { sleep 5; };			
		{
			...
			[_x,_bld,_bldPos,_wait,[_behave,_combat,_speed,_formation]] spawn UO_AI_fnc_taskBuildingDefend;
		} forEach (units _grp);
	};
	if(UO_FW_AI_DEBUG) then {
		if(_patrol) then {
			...
			[_grp,_bpos,"rBldD"] spawn UO_AI_fnc_debugCreateMarker;
		};
	};
	true